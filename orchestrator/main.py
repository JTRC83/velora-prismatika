...
# Esta lista centraliza todo. Si creas un servicio nuevo, solo lo añades aquí.
SERVICE_LIST = [
...
for service_name, module_path in SERVICE_LIST:
    try:
        ...
        # Opcional: print(f"[✨ Velora] Canalizado servicio: {service_name}")
    except Exception as e:
        print(f"[⚠️ Velora Warning] No se pudo despertar el servicio {service_name}: {e}")

# 3. Router de Orquestación Principal
# Lo incluimos al final para que los servicios específicos tengan prioridad
app.include_router(orchestrator_router)

# 4. Punto de Entrada Raíz
@app.get("/")
def root():
    return {
        "entidad": "Velora Prismätika",
        "estado": "Despierta",
        "mensaje": "El portal se ha abierto. El conocimiento fluye a través de los símbolos."
    }